...
// FIX: Changed to default import for JSZip and will use it directly.
...
const App: React.FC = () => {
  ...
  // FIX: Updated default model to a more capable version for code generation.
...
<body>
  ...
  const handleSendMessage = useCallback(async (prompt: string) => {
    ...
    try {
      const result = await generateWebsite(prompt, uploadedFiles);
      if (result) {
  ...
  const handleDownloadZip = useCallback(async () => {
    ...
    // FIX: Use the imported JSZip module directly.
    const zip = new JSZip();
