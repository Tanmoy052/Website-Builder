...
export const Sidebar: React.FC = () => {
  return (
    <aside className="studio-sidebar flex flex-col h-full border-r border-studio-border bg-white">
      <div className="p-4 border-bottom border-studio-border">
      ...
      <nav className="flex-1 overflow-y-auto p-2 space-y-1">
        ...
        <SidebarItem icon={<MessageSquare size={18} />} label="AI Chat" active />
...
const SidebarItem: React.FC<SidebarItemProps> = ({ icon, label, active }) => {
  return (
    <button className={`flex items-center gap-3 w-full px-3 py-2 rounded-md transition-colors text-sm font-medium ${
      active ? 'bg-blue-50 text-studio-accent' : 'text-gray-600 hover:bg-gray-100'
    }`}>
      {icon}
      <span>{label}</span>
    </button>
